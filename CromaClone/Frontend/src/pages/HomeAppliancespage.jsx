...
const brands = ["All", "LG", "Samsung", "Whirlpool", "IFB", "Haier", "Voltas", "Dyson"];
...
export default function HomeAppliancesPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const data = await getProducts("Home Appliances");
        setProducts(data);
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] pt-10 pb-20">
      ...
      <HSlider title="Featured Home Appliances" subtitle="Top Picks" accent={accentColor} loading={loading} viewAllPath="/products/appliances/featured">
        {featuredProducts.map((p) => <FeaturedCard key={p._id} p={{ ...p, id: p._id }} />)}
      </HSlider>

      <HSlider title="Best Deals & Offers" subtitle="Limited Time" accent="#f97316" loading={loading} viewAllPath="/products/appliances/deals">
